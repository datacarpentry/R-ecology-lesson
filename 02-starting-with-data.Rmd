...
Before you use a package for the first time you need to install it on your machine, and then you
should import it in every subsequent R session when you need it. 

To install the package, we can type `install.packages("tidyverse")` straight into the console. In fact, it's better to write this in the console than in our script for any package, as there's no need to re-install packages every time we run the script.
...
name: `surveys`. 
By default, this will show show you as many rows and columns of the data as fit
on your screen. If you wanted to the first 50 rows, you could type 
`print(surveys, n = 50)`

We can extract the first few lines of this data using the
function `head()`:
...
use this assign the first 100 rows of `surveys` to an object using 
`surveys_sample <- head(surveys, 100)`. This can be useful if you
want to try out complex computations on a subset of your data before you apply
them to the whole data set.
...
To open the data set in RStudio's Data Viewer, use the `view()` function:
...
which is a special kind of `data.frame`. 
...
data frame with some tweaked behavior. For our purposes the important 
...
1. In addition to displaying the data type of each column under its name, it
   ...
   screen. If you want to see more rows, you can use the `print()` function 
2. Columns of class `character` are never converted into factors. This is 
   different from the default behavior of R in versions prior to 4.0.
...
the figure, we can turn the missing values into a factor level with the 
