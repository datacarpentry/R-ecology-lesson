...
> ### Challenge
>
> 1. Create a `data.frame` containing only the observations from row 1999 of the
>    `surveys` dataset.
>
> 2. Notice how `nrow()` gave you the number of rows in a `data.frame`? Use
>   `nrow()` instead of a row number to make a `data.frame` with observations from
>   only the last row of the `surveys` dataset.
>
...
>   `head(surveys)` excluding the 7th through final row of the `surveys` dataset.

```{r, echo=FALSE, purl=TRUE}
### Challenges:
###
###  1. Create a `data.frame` (`surveys_1999`) containing only the
###    observations from row 1999 of the `surveys` dataset.
###
### 2. Notice how `nrow()` gave you the number of rows in a
###   `data.frame`? Use `nrow()` instead of a row number to make a
###   `data.frame` (`surveys_last`) with observations from only the
###   last row of the `surveys` dataset.
###
### 3. Use `nrow()` in conjunction with the `:` function to create a new
###    `data.frame` called `surveys_last_100` that includes the 100 rows of the
###    survey data frame.
...
of a special class called a `factor`. Factors are very useful but not
necessarily intuitive, and therefore require some attention.
...
label to something more meaningful. Before doing that, we are going to create a
new object so we don't loose the original data:
