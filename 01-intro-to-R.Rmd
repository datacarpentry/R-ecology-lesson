...
> * Call functions and define them with arguments and their options.
...
(where you put spaces, how you name variable, etc.). Using a consistent coding
style, make your code clearer to read for your future self and your
collaborators. In R, two popular style guides
are [Hadley Wickham's](http://adv-r.had.co.nz/Style.html)
and [Google's](https://google-styleguide.googlecode.com/svn/trunk/Rguide.xml)
...
the workhorse of R. A vector is composed by a series of values, which can be   either numbers or characters. We can assign a series of values to a vector using the `c()` function, with the values to combine in the vector as its argument. For example we can create a vector of animal weights and assign it to a new object `weight_g`:
...
will assume there are objects called mouse, rat and dog and launch and error message as it can not find them.
...
We just saw 2 of the 6 main **atomic vector** types (or simply **data types**) that R uses: `"character"` and
...
A particular and very useful data type is `"factor"` which encodes categorical variables (e.g. Habitat types, Gender etc.) 
