...
```{r, purl=FALSE}
3 + 5 ###### Put in an actual print screen image of the input and output of the R console. Example attached.
12/7
```
...
equal to", and `==` for "equal to". The double equal sign `==` should not be confused 
with the single `=` sign, which assigns to a variable (roughly speaking the same as
"<-").
...
you may overlook the fact that you are dealing with missing data. You can add the
