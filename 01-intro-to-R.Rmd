...
```{r, purl=FALSE}
heights[!is.na(heights)] #extract those elements which are not missing values
na.omit(heights) #returns the object with incomplete cases removed. the returned object is atomic
heights[complete.cases(heights)] #extract those elements which are complete cases
```
