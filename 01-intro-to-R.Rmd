...
> ### Learning Objectives
>
> * Define the following terms as they relate to R: script, function, assign, object, variable.
> * Call functions with zero or more named or unnamed arguments.
> * Perform mathematical computations in R.
...
instance, we may want to convert this weight to pounds (weight in pounds is 2.2
...
> ### Call out
>
> What are called `objects` in `R` are known as `variables` in many other programming languages.
...
would for one item. (In fact the basic variables we used previously are simply 
vectors with just a single element.)

For example we can create a vector that contains 4 animal weights:
...
In the first line, what happens is that we take the original vector `weight_g`, 
adding the value `90` to the end of it, and saving the result back into 
...
> ### Challenge
>
> 1. We’ve seen that atomic vectors can be of type character,
...
> 
> 4. You've probably noticed that objects of different types get
...
## Answers
##
## 1. R implicitly converts them to all be the same type
## 2. `num_char` will be a vector of characters,
##    `num_logical` will be a vector of numerics, 
##    `char_logical` will be a vector of characters, 
##    `tricky` will be a vector of characters
## 3. Vectors can be of only one data type. R tries to convert (coerce)
##    the content of this vector to find a "common denominator".
## 4. logical -> numeric -> character <- logical
```
--->
...
```{r, purl=FALSE}
## Answers
## 1. When using ">" or "<" on strings, R compares their alphabetical order. Here
...
## Answers
##
## 1. finite_lengths <- lengths[!is.NA(lengths)]
## 2. median(lengths, na.rm = TRUE)
```
--->
