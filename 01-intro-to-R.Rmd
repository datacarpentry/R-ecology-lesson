...
Question: We’ve seen that atomic vectors be of type character, numeric, integer, logical, and complex. But what happens if we try to mix these types in a single vector?
Answer: R implicitly converts them to all be the same type

Question: What do you think the type of each of these vectors will be?
...
Question: Can you work out what the rule is?
Answer: from the docs: “The output type is determined from the highest type of the components in the hierarchy NULL < raw < logical < integer <  double < complex < character < list < expression.” More informatively, R tries not to lose any information, so it converts e.g. numerics into characters because every numeric can be represented as a character, but not vice-versa.
