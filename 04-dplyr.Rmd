...
`surveys_clean` has `r nrow(surveys_clean)` rows and `r ncol(surveys_clean)`
columns by typing `dim(surveys_clean)`.
...
write.csv(surveys_clean, file="data_output/surveys_clean.csv",
          row.names=FALSE)
```
