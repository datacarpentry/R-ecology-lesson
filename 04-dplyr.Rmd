...
**`tidyr`**, enabling you to swiftly convert between different data formats for
plotting and analysis.

Packages in R are basically sets of additional functions that let you do more
stuff. The functions we've been using so far, like `str()` or `names()`,
come built into R; packages give you access to more of them. Before you use a
package for the first time you need to install it on your machine, and then you
should import it in every subsequent R session when you need it. You should
already have installed the **`tidyverse`** package. This is an "umbrella"
package that installs several packages useful for data analysis which work
together well, such as **`tidyr`**, **`dplyr`**, **`ggplot2`**, **`tibble`**,
an others.
...
2. Using R expressions in a non standard way often can be confusing for new
   learners.
3. New learners are often not aware of hidden arguments or function defaults.
...
We'll read in our data using the `read_csv()` function, from the tidyverse package **`readr`**, instead of `read.csv()`. The package `read_csv()` has some fundamental differences compared to the built-in `read.csv()` function. One of the authors of the `tidyverse()` and `readr` packages, Hadley Wickham, outlines the advantages over the base equivalents being:
> * the `readr()` packages tend to run more quickly.
> * the `reard()` packages produce tibbles, a special type of data frame that we will cover shortly.
> * the `reard()` packages do not inherit behaviors from the operating system and environment variables like the base R functions, which allows for better reproducibility across different computer systems.

```{r, results = 'hide', purl = FALSE}
surveys <- read_csv("data/portal_data_joined.csv")

## print the data abbreviation
surveys
...
Notice that the class of the data `tbl_df` and `tbl` in addition to `data.frame`. This is referred to as a "tibble". For those of you who may have seen generic data frames before this course, tibbles tweak some of the behaviors of the basic `data.frame`. For our purposes, the only differences are that:

1. **Printing:** In addition to displaying the data type of each column under its name, it
   only prints the first few rows of data and only as many columns as fit on one
   screen.
2. **Subsetting:** Compared to traditional data frames, tibbles simplify the way in which you can subset data by name or position. Tibbles can also utilize pipes, a useful coding tool, which we will cover a little later.
3. **Changes to Data:** Tibbles never convert `character` columns into factors, never change names of variables, and never create row names.
