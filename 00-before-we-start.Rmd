...
data science.](https://www.tidyverse.org/)" We will be using the packages within `tidyverse` to go through
...
Packages can also be used to contain datasets. For this lesson we will use the `ratdat` data package as a "[real world example of life-history, population, and ecological data ... with many complexities removed to allow students to focus on the core ideas and skills being taught.](https://github.com/weecology/ratdat)" 

Since `ratdat` is hosted on github, we will need the `devtools` package, which contains an interface between `R` and `github`, along with other useful tools. To install `devtools`, run the following code in your R **Console** panel.
...
Then to download the data, we load the `devtools` package and use it to install the `ratdat` package, run the following code, once again in your R Console panel:
