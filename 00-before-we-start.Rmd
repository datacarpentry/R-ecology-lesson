...
and clicking but instead in a series of written commands, and that's a good
...
developing packages, writting Shiny apps) that we will not cover during the
workshop.

![RStudio interface screeshot](img/rstudio_screenshot.png)
...
the <kbd>`Ctrl`</kbd> + <kbd>`Enter`</kbd> shortcut (on Macs, <kbd>`Cmd`</kbd> + <kbd>`Return`</kbd> will work, too). The command on the current line in the 
...
When this happens, and you thought you finished typing your command, click inside
the console window and press `Esc`; this will cancel the incomplete command and
...
[rdocumentation.org](http://www.rdocumentation.org) website that searches through
...
The key to receiving help from someone is for them to rapidly grasp your problem. You
