...
that contains the survey data from figshare, and we will use `read_csv()` from
...
Load the **`tidyverse`** for the `read_csv()` function:
...
The `read_csv()` function prints a message informing us on its best guess of
...
that our data has been loaded properly, we can see the contents of the data
frame by typing its name:

```{r, eval=TRUE, purl=TRUE, results='markup'}
surveys
```

In this output, `R` tells us that the `surveys` object is "a tibble". Tibble data
...
Additionally, you can use the `head()` and `tail()` functions to inspect the top
or bottom rows of your data frame, respectively.
...
> the `read_tsv()` for importing tab-separated data files. It is important to
> note that all of these functions are actually wrapper functions for the main
> `read_delim()` function with different arguments. As such, the surveys data
> above could have also been loaded by using `read_delim()` with the `delim`
> argument as `,`. The code is as follows:
> `surveys <- read_delim(file = "data/portal_data_joined.csv", delim = ",")`.
...
functions `read_csv()` or `read_delim()`; in other words, when importing
...
> ### Challenge
>
> Based on printing `surveys` and the output of `str(surveys)`, can you answer
...
date is stored as a separate variable. By printing `surveys`, we can confirm
that our data frame has a separate column for day, month, and year, and that
...
```{r, purl=FALSE}
characterDates <- paste(surveys$year, surveys$month, surveys$day, sep = "-")
head(characterDates)
```

This character vector can be used as the argument for `ymd()`:

<!-- Use `warning = FALSE` to get around the issue with date 129: -->
<!-- Warning message: -->
<!--  129 failed to parse.  -->
```{r, purl=FALSE, warning=FALSE}
dates <- ymd(characterDates)
head(dates)
```
