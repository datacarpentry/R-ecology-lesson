...
R can connect to databases through a number of packages.  In our case we we will use [RSQLite](http://cran.r-project.org/web/packages/RSQLite/index.html) to connect to existing SQLite3 databases.  However you should be able to connect on almost any database in R via JDBC[http://cran.r-project.org/web/packages/RJDBC/index.html](http://cran.r-project.org/web/packages/RJDBC/index.html) or [ODBC](http://cran.r-project.org/web/packages/RODBC/index.html), or specific database packages (such as we are doing, or [MySQL](http://cran.r-project.org/web/packages/RMySQL/index.html) ).  

To begin these exercises we'll connect to the mammal database you've just created. We'll need the RSQLite library to be sure to install it first. `install.packages('RSQLite')`.

It's also important that your working directory is set properly. Check it first `getwd()`. Set your working directory if it's not set to the same data directory in your data-carpentry folder that we've been working with.
...
The `RSQLite` library also has functions that can be used to list both tables and fields within a table. Here you can see the types and names of fields and get a count of records.
...
Field Aliases in SQL
----
********** ALIASING IS UNECCESARY HERE BUT IS A GOOD TOPIC TO KNOW ********
