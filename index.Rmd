...
`Tools > Check for Package Updates...`. Sometimes, package updates introduce
changes that break your old code, which can be very frustrating.
To avoid this problem, you can use a package called **`renv`**. It locks the
package versions you have used for a given project and makes it straightforward
to reinstall those exact package version in a new environment, for example on
another computer.

R itself also occasionally receives updates. These are much less likely to break
your code, but it may still happen. However, it is possible to have multiple
versions of R installed in parallel, and you can switch between them in RStudio
by going to `Tools > Global Options > General > Basic`.

After installing a new version of R, you will have to reinstall all your packages
with the new version. Again the **`renv`** package can make this process easier.
For Windows, there is also the **`installr`** package that can help you with
upgrading your R version and migrate your package library.

Finally, you can check for updates of RStudio by going to `Help > Check for Updates`
and following the instructions on screen. By default, RStudio will also 
automatically check for updates every once in a while.
