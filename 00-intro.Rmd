...
<script>
doclick=function(e){
e.nextSibling.nextSibling.style.display = e.nextSibling.nextSibling.style.display == "none" ? "block" : "none";
...
<div class="hidecode" onclick="doclick(this);">[Solution]</div>
```r
# Solution #
...
`ggplot2` is a plotting package that makes it sipmple to create complex plots from data
in a dataframe. It uses default settings, which help creating publication quality plotts with minimal amount of settings and tweaking.
...
To add a ageom to the plot use `+` operator:
```r
ggplot(surveys_complete, aes(x=weight, y=hindfoot_length)) +
            geom_point()
```
...
Example of complex visualisation in which plot area is divided into
hexagonal sections and points are counted wihin hexagons.
The number of points per hexagon is encoded by color.
```r
ggplot(surveys_complete, aes(weight, hindfoot_length)) +
            stat_binhex(bins=50) +
...
#### Boxplot
Visualizing the distribution of weight within each species.
```r
...
Lets calculate number of counts per year for each species.
...
Timelapse data can be visualized as a line plot with years on x axis and counts on y axis.
```r
ggplot(yearly_counts, aes(x=year, y=count)) +
                  geom_line()
```
Unfortunatelly this does not work, because we plot data for all the species together. We need to tell ggplot to split graphed data by `species_id`
```r
ggplot(yearly_counts, aes(x=year, y=count,
                          group=species_id)) +
                  geom_line()
...
Now we wuld like to split line in each plot by sex of each individual measured.
To do that we need to make counts in dataframe grouped by sex.

### Challenge:
* filter the dataframe so that we only keep records with sex "F" or "M";

hint: look at example where we remove species with lower than 10 counts.
...
```
### Challenge:
* make the faceted plot spliting further by sex
(within single plot)

hint: use group (like in the plot where we group per species)
...
We can improve the plot by coloring by sex instead of species
(species are already in separate plots, so we don't need to distinguish them better)
```r
ggplot(yearly_sex_counts, aes(x=year,
                             y=count,
                             color=sex,
