...
source("setup.R")
library(tidyverse)
surveys_complete <- read_csv(
  file = "data_raw/portal_data_joined.csv",
  col_types = cols()
) %>%
...
```{r scatter-challenge-answer, answer=TRUE}
ggplot(data = surveys_complete, 
...
```{r boxplot-challenge, eval=FALSE, echo=FALSE, purl=TRUE}
...
```{r final-challenge, eval=FALSE, echo=FALSE, purl=TRUE}
...
```{r patchwork-example, message=FALSE, fig.width=10}
library(patchwork)
