...
To run an entire script you can use the <kbd>`Ctrl`</kbd> +  <kbd>`Shift`</kbd> + 
<kbd>`Enter`</kbd> shortcut (on Macs, <kbd>`Cmd`</kbd> + <kbd>`Shift`</kbd> + 
<kbd>`Return`</kbd>) or use the "Source" toolbar button on the 
right top corner of the source pane.
