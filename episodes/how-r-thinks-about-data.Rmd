...
Let's get back to investigating our `complete_old` data.
frame.
...
First, we are told that we have a data.
frame of `r nrow(complete_old)` observations, or rows, and `r ncol(complete_old)` variables, or columns.
...
This is because the `$` is an operator that allows us to select individual columns from a data.
frame.

The `$` operator also allows you to use tab-completion to quickly select which variable you want from a given data.
frame.
...
You might have noticed that our last result looked different from when we printed out the `complete_old` data.
frame itself.
That's because it is not a data.
frame, it is a **vector**.
A vector is a 1-dimensional series of values, in this case a vector of numbers representing years.

Data.
frames are made up of vectors; each column in a data.
frame is a vector.
...
Since each column in a data.
frame is a vector, this means an accidental character following a number, like `29,` can change the type of the whole vector.
...
We have now seen vectors in a few different forms: as columns in a data.
frame and as single vectors.
