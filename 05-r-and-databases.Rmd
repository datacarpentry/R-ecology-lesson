...
extended to other database types.

Interfacing with databases using `dplyr` focusses on retrieving and analyzing
datasets by generating `SELECT` SQL statements, but it doesn't modify the
database itself.  For example, dplyr does not offer functions to `UPDATE` or
`DELETE` entries. If you need these functionalities, you will need to use the
dedicated R packages (e.g., RSQLite).

Here we will demonstrate how to interact with a database using dplyr, using both
the dplyr's verb syntax and the SQL syntax.
...
Now that we know that we can connect to the database, let's explore how to get
the data from its tables in R.

### Querying the database with the SQL syntax

To connect to tables within a database, you can use the `tbl()` function from
dplyr. This function can be used to send SQL queries to the database. For
instance, if you wanted to select the columns "year", "species_id", and
"plot_id" from the `surveys` table, you could use the SQL syntax:

```{r use-sql-syntax}
tbl(mammals, sql("SELECT year, species_id, plot_id FROM surveys"))
```

With this approach you can use any of the SQL queries we have covered in the
database lesson.

### Querying the database with the dplyr syntax

One of the strengths of dplyr, is that the same operation can be done using
using the dplyr's verbs instead of writing SQL. First, we select the table on
which to do the operations, and then we use the standard dplyr syntax:

```{r use-dplyr-syntax}
surveys <- tbl(mammals, "surveys")
surveys %>%
    select(year, species_id, plot_type)
```

In this case, the `surveys` object behaves like a data frame, and several of the
functions that can used with data frames, can also be used on tables from a
...
into memory, and when your code is ready, you can change the input dataset as
...
> multiple tables together uusing the following syntax where foreign key refers
