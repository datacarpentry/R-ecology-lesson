...
suppressWarnings(surveys$date <- lubridate::ymd(
  paste(
    surveys$year,
    surveys$month,
    surveys$day,
...
## each `year` for each different `plot_type`. Then `spread()` them into a data 
## set with a column for `hindfoot_length` and `weight`. *Hint*: You only need 
## to specify the key and value columns for `spread()`.
```
...
```{r, results="hide", echo=FALSE, purl=TRUE}
