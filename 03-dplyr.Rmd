...
Using `pivot_wider()` using the names from the `genus` variable and with values from `mean_weight` this becomes
...
although we may wish to fill in the missing values first (the "NA" values will be replaced with 0).
...
To recreate `surveys_gw` from `surveys_wide` we would create a key (names variable) called
`genus` and a value variable called `mean_weight`. We will use all columns except `plot_id` for the names variable. Here we exclude `plot_id` from being `gather()`ed.
