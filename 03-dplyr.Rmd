...
```
#In order to see the mutated results, it may be useful to create an intermediate object since it is not displayed otherwise.

weight_mod <- surveys %>% mutate(weight_kg = weight/1000)
...
that escaped before their sex could be determined and weighed. You may notice
...
multiple rows, 196 observations of 3 variables in 13 columns. 
...
surveys_complete <- surveys_complete %>%
  filter(species_id %in% species_counts$species_id) #There should be an explanation of what this modified pipe is.  %in% is not introduced anywhere else in the lesson.
```
